    ...
    : QMainWindow(parent)
{
    ...
    QRadioButton *taskVariant1 = new QRadioButton("Зачёт", this);
    QRadioButton *taskVariant2 = new QRadioButton("Лаба", this);
...
void MainWindow::showTrueGraph(){
    ...
    if(selectedTask == Functions::STRONGINA){
        true_u = u_real::u;
    } else if(selectedTask == Functions::KAPKAEV) {
        true_u = u_kapkaev::u;
    }
...
void MainWindow::showGraph() {
    ...
    if (Xn != slv.N || Yn != slv.M || maxN != slv.max_it || epsilon != slv.epsilon || selectedTask != slv.task) {
...
void MainWindow::onRadioButtonClicked(){
    ...
    if (selectedButton->text() == "Зачёт") {
        slv = solver(Functions::STRONGINA);
        selectedTask = Functions::STRONGINA;
        a = 0;
        b = 1;
        c = 0;
        d = 1;
    } else if (selectedButton->text() == "Лаба") {
        slv = solver(Functions::KAPKAEV);
        selectedTask = Functions::KAPKAEV;
        a = -1;
        b = 1;
        c = -1;
        d = 1;
