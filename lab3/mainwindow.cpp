    ...
    if (selectedTask == Functions::test){
        ...
        for (int j = 0; j <= 2 * Yn; j++) {
            ...
            for (int i = 0; i <= 2 * Xn; i++) {
                row->append(QSurfaceDataItem(QVector3D(a + (h / 2) * i * skipx, v2(i  * skipx, j * skipy), c + (k / 2) * j * skipy)));
            }
            dataTrueSeries->dataProxy()->addRow(row);
        }
...
void MainWindow::drawGraph(){
    ...
    for (int j = 0; j <= Yn; j++) {
        QSurfaceDataRow* row = new QSurfaceDataRow;
        for (int i = 0; i <= Xn; i++) {
            row->append(QSurfaceDataItem(QVector3D(a + slv.h * i * skipx, v(i  * skipx, j * skipy),
                                         c + slv.k * j * skipy)));
        }
            ...
            for(size_t t = 0; t < 10; t++){
                for (int j = 0; j <= slv.M; j++) {
                    ...
                    for (int i = 0; i <= slv.N; i++) {
                        row->append(QSurfaceDataItem(QVector3D(a + slv.h * i, vPhotos[t](i, j), c + slv.k * j)));
                    }
            ...
            if (Xn < 100 && Yn < 100) {
                ...
                for(size_t t = 0; t < 10; t++){
                    for (int j = 0; j <= slv.M; j++) {
                        QSurfaceDataRow* row = new QSurfaceDataRow;
                        for (int i = 0; i <= slv.N; i++) {
                            row->append(QSurfaceDataItem(QVector3D(a + slv.h * i, vPhotos[t](i, j),
                                                         c + slv.k * j)));
                        }
                        iterations[t]->dataProxy()->addRow(row);
                    }
                ...
                for(size_t t = 0; t < 10; t++){
                    for (int j = 0; j <= slv2.M; j++) {
                        ...
                        for (int i = 0; i <= slv2.N; i++) {
                            row->append(QSurfaceDataItem(QVector3D(a + slv2.h * i, v2Photos[t](i, j),
                                                         c + slv2.k * j)));
                        }
...
void MainWindow::showSummary(){
    ...
    if(slv.task == Functions::test){
        ...
        for(int i = 0; i <= slv2.iter.size(); i++){
            ...
            tableWidget4->setItem(4, i, new QTableWidgetItem(QString::number(slv2.ACCURACY[i])));
            tableWidget4->setItem(5, i, new QTableWidgetItem(QString::number(slv2.MAX_R[i])));
        }
    }
...
void MainWindow::showPGraph(){
    ...
    if (selectedTask == Functions::test){
        ...
        for (int j = 0; j <= Yn; j++) {
            QSurfaceDataRow* row = new QSurfaceDataRow;
            for (int i = 0; i <= Xn; i++) {
                row->append(QSurfaceDataItem(QVector3D(a + slv.h * i * skipx, z(i * skipx, j * skipy), c + slv.k * j * skipy)));
            }
    ...
    if (selectedTask == Functions::tmain){
        ...
        for (int j = 0; j <= Yn; j++) {
            QSurfaceDataRow* row = new QSurfaceDataRow;
            for (int i = 0; i <= Xn; i++) {
                row->append(QSurfaceDataItem(QVector3D(a + slv.h * i * skipx, v(i  * skipx, j * skipy) - v2(2 * i * skipx, 2 * j * skipy), c + slv.k * j * skipy)));
            }
            dataPSeries->dataProxy()->addRow(row);
        }
        dataPSeries->setItemLabelFormat("solution @xLabel @yLabel @zLabel");
